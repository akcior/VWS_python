...
class Okno_sym(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        pole_narratora = tk.Text(self, yscrollcommand=scrollbar.set)
        pole_narratora.config(width=480)
        # for i in range(1000):
        #     pole_narratora.insert(tk.END, str(i) +"\n")
        plansza.ustaw_pole_narratora(pole_narratora)
        scrollbar.config(command=pole_narratora.yview, width=30)
        pole_narratora.pack(side=tk.LEFT, expand=0)#grid(row=0, column=2)
        scrollbar.pack(side=tk.RIGHT,fill=tk.Y)#grid(row=0, column=3)
